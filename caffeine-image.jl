...
for line in readlines("/home/jerboa/caffiene.xyz")[3:end]
...
f = Figure(size=(1920,1080),fxaa=true, figure_padding=1)
a = Axis3(f[1, 1])
pixelColours, rayColours, rays, rayLighting, rayIntersections = pixelGrid!(a, positions, radii, colours, Point3f(p0), gridPoints=25, gridSize=Float32(0.025))

gridPoints=25
rayOrigin=p0
gridSize = Float32(0.1)
gridOrigin = Point3f(0,0,0)
quadDirection=Point3f(0,0,1)
theta = Float32(pi)
base, q_a, q_b = quadVertices(quadDirection, gridOrigin, theta, gridSize)
q_a = Point3f(q_a*gridSize)
q_b = Point3f(q_b*gridSize)
...
for i in -gridPoints+1:gridPoints-1
    for j in -gridPoints+1:gridPoints-1
        pos = gridOrigin+q_a*i+q_b*j
        quad!(ax, quadDirection, position=pos, theta=theta, scale=gridSize, colour=pixelColours[k])
        global k+=1
    end
end
...
times = range(0.0, rayTime*5, step=1.0/fps)
dt = 1.0/fps
ax.elevation[] = -pi/2.0
ax.azimuth[] = pi
